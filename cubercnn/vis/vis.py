...
def visualize_from_instances(detections, dataset, dataset_name, min_size_test, output_folder, category_names_official, prompted_category_names, iteration='', ):
    """
    Visualize dataset detections in three columns:

    Ground truth annotations for all classes
    Ground truth annotations for evaluated classes (could be both base and novel classes according to the evaluation setting)
    OVMono3D-LIFT predictions for evaluated classes

    Each columns displays:

    2D detections (upper row)
    3D detections (lower row)    
    """
        ...
        for instance in im_obj['instances']:
            ...
            if valid_match:
                ...
                try:
                    ...
                    pass
            if write_sample:
                # Draw the predicted 2D bounding box
                color = util.get_color(instance['category_id'])
                cv2.rectangle(im_pred_2d, (int(x1), int(y1)), (int(x2), int(y2)), color, 2)
                # Add category label
                cv2.putText(im_pred_2d, cat, (int(x1), int(y1) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)

                # unproject point to 3D
                x3d, y3d, z3d = (K_inv @ (z3d*cen_2d))

                # # Draw Ground Truth 3D bounding box
                # if cat in prompted_category_names:
                #     draw_3d_box(im_gt_3d, K, [gt_x3d, gt_y3d, gt_z3d, gt_w3d, gt_h3d, gt_l3d], gt_ry3d, color=color, thickness=int(np.round(3*im.shape[0]/500)), draw_back=False)
                #     draw_text(im_gt_3d, cat, [gt_x1, gt_y1, gt_w, gt_h], scale=0.50*im.shape[0]/500, bg_color=color)

                # Draw predicted 3D bounding box
                if score > thres:
                    draw_3d_box(im_pred_3d, K, [x3d, y3d, z3d, w3d, h3d, l3d], ry3d, color=color, thickness=int(np.round(3*im.shape[0]/500)), draw_back=False)
                    # Add category label to the top-left corner of the 3D box.
                    draw_text(im_pred_3d, cat, [x1, y1, w, h], scale=0.50*im.shape[0]/500, bg_color=color)
        if write_sample:
            top_row = cv2.hconcat([im_gt_all_classes_2d, im_gt_2d, im_pred_2d])
            bottom_row = cv2.hconcat([im_gt_all_classes_3d, im_gt_3d, im_pred_3d])
                ...
                if ground_bounds is None:
                    ...
                    if (not maskz.any()) or (not maskx.any()):
                        if mode == 'front_and_novel':
                            return im, im_novel_view, canvas
                        else:
...
def imvstack(im1, im2):
    ...
    if sf > 1:
        im2 = cv2.resize(im2, (im1.shape[1], int(im2.shape[0] * sf)))
    elif sf < 1:
        im1 = cv2.resize(im1, (im2.shape[1], int(im1.shape[0] / sf)))

    im_concat = cv2.vconcat([im1, im2])
